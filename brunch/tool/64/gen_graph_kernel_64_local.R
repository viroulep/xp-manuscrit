...
framegemm = read.table("./summarized_all_data_dgemm_64.dat", header=TRUE)
avg_dgemm = subset_ddply(framegemm, 64)

frametrsm = read.table("./summarized_all_data_dtrsm_64.dat", header=TRUE)
avg_dtrsm = subset_ddply(frametrsm, 64)

framepotrf = read.table("./summarized_all_data_dpotrf_64.dat", header=TRUE)
avg_dpotrf = subset_ddply(framepotrf, 64)

framesyrk = read.table("./summarized_all_data_dsyrk_64.dat", header=TRUE)
avg_dsyrk = subset_ddply(framesyrk, 64)
...
pdf("kernel_64_local_idchire.pdf", width = 10, height=6)
